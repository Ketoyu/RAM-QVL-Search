	...
	public partial class Form1 : Form
	{
		...
		class Kit
		{
			...
			public static implicit operator ListViewItem(Kit kit) {
				return new ListViewItem(new string[] {kit.Vendor, kit.PartNo, "" + kit.Size_Stick + "G x" + kit.Sticks + " = " + kit.Size_Total + "G", kit.SS_DS,
					kit.Chip, kit.Timings_Text, StringsMisc.Dbl_ToString(Math.Round(kit.Voltage, 2)).RemoveTrailing('0'), "" + kit.Speed, "" + kit.Speed_Ryzen3,
					(kit.Dimms[1] ? "•" : ""), (kit.Dimms[2] ? "•" : ""), (kit.Dimms[4] ? "•" : "") });
		...
		private void btnPaste_Click(object sender, EventArgs e)
		{
			...
			Search.Vendors = (from Kit kit in Kits orderby kit.Vendor select kit.Vendor).GroupBy(i => i).Select(g => g.First()).ToList();
			Search.StickSizes = (from Kit kit in Kits orderby kit.Size_Stick select kit.Size_Stick + " GB").GroupBy(i => i).Select(g => g.First()).ToList();
			Search.SS_DS = (from Kit kit in Kits orderby kit.SS_DS descending select kit.SS_DS).GroupBy(i => i).Select(g => g.First()).ToList();
			Search.Chips = (from Kit kit in Kits orderby kit.Chip select kit.Chip).GroupBy(i => i).Select(g => g.First()).ToList();
			Search.Timings = (from Kit kit in Kits orderby kit.CL select kit.CL).GroupBy(i => i).Select(g => g.First()).ToList();
			Search.Speeds = (from Kit kit in Kits orderby kit.Speed descending select kit.Speed).GroupBy(i => i).Select(g => g.First()).ToList();
			Search.Speeds_Ryzen3 = (from Kit kit in Kits orderby kit.Speed_Ryzen3 descending select kit.Speed).GroupBy(i => i).Select(g => g.First()).ToList();
			...
			void fillGroupbox<T>(GroupBox gb, List<T> lst) {
				...
				for (int i = 0; i < lst.Count; i++) {
					...
					gb.Width = classMath.Higher(gb.Width, chk.Width + 15);
				}
				x += gb.Width + 15;

				this.Height = classMath.Higher(this.Height, gb.Location.Y + 32 + gb.Height + 15);
			}
